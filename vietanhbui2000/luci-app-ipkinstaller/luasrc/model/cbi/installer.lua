...
um.template = "cbi/installer_dvalue"
...
dm.template = "cbi/installer_dvalue"
...
mt = tb:option(DummyValue, "mtime", translate("Modified time"))
ms = tb:option(DummyValue, "modestr", translate("File attributes"))
sz = tb:option(DummyValue, "size", translate("File size"))
...
btnis.template = "cbi/installer_button"
...
btnis.write = function(self, section)
	local r = luci.sys.exec(string.format('opkg install "/tmp/%s"', inits[section].name))
